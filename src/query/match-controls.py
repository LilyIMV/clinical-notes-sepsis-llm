...
all_case_ids = cases['icustay_id'].unique() # get unique ids for ratio


# -----------------------
# EDIT: Apply 7-hour sepsis filter (now adjusted in hourly-cohort)
# -----------------------
# cases = cases[cases['sepsis_onset_hour'] >= 7].reset_index(drop=True)

case_ids = cases['icustay_id'].unique() # get unique ids
control_ids = controls['icustay_id'].unique()

#--------------------------------
# 2. Determine Control/Case Ratio (Unfortunately cannot be calculated as the sepsis cases are further filtered in hourly-cohort.sql. I has been previouslt tested and rf =10)
#--------------------------------
rf = 10
...
if args.rigorous:
    ...
    for i, case_id in enumerate(case_ids):
        ...
        onset_hour = float(cases[cases['icustay_id'] == case_id]['sepsis_onset_hour'].iloc[0]) # get float of current case onset hour
        ...
        result = pd.concat([result, matched_controls], ignore_index=True)
...
print(f'Total Matched Controls: {len(result)}')
print(f'Expected: {len(cases)} × {rf} = {len(cases) * rf}')
