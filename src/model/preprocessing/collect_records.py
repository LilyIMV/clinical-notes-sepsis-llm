...
def collect_records(filepath=None, outpath=None):
    ...
    variable_indices = range(5, len(parts))
    variables = parts[5: len(parts)] #containing the medical variable names
    ...
    tmp_values = parts[5:len(parts)] # get list of all medical values as each as string
    ...
    for i in range(len(current_values)):
        if tmp_values[i] != '':
            try:
                ...
                print(f"ERROR: Cannot convert '{tmp_values[i]}' to float in column {i}")
                print(f"Row: {parts}")
                sys.exit(1)
